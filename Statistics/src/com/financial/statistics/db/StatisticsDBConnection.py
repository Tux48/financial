...
com.financial.common.db.MySqlDBConnection -- Statistics数据库连接类

com.financial.common.db.MySqlDBConnection is a 
Statistics数据库连接类，此类是一个单例类。

It defines classes_and_methods     def __initDB( self ):                    初始化数据库连接池
                                                    def getStatisticslDBSession( self ):    从数据库连接池中返加一个连接
...
class StatisticsDBConnection( object ):
    ...
    def __new__( cls, *args, **kwargs ):
        if not hasattr( StatisticsDBConnection, "_instance" ):
            with StatisticsDBConnection.__instance_lock:
                if not hasattr( StatisticsDBConnection, "_instance" ):
                    StatisticsDBConnection._instance = object.__new__( cls )
                    
        return StatisticsDBConnection._instance
    ...
    def __initDB( self ):
        ...
        self.statisticsDBSession  = scoped_session( sessionmaker( bind = mysqlDBEngine, autocommit = isAutoComit, autoflush = isAutoFlush, 
                                                             expire_on_commit = isExpireOnComit ) )
        
    '''
    @summary: 从数据库连接池中返加一个连接
    '''
    def getStatisticsDBSession( self ):
        dbSession = self.statisticsDBSession()
